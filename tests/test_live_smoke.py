...
def _airwallex_issuing_or_skip(crude_config):
    """A client for an Issuing read, skipping when creds or the product are absent.

    The `issuing` group needs the Issuing product enabled on the account; a disabled
    account answers ``403 access_denied_not_enabled`` (verified live on a production
    account without issuing), which is a skip (nothing to test here), not a failure.
    An enabled account answers with a (possibly empty) page.
    """
    if not crude_config.get("airwallex", {}).get("client_id"):
        pytest.skip("no [airwallex] credentials in config")
    from crude_airwallex.cli import _make_client
...
@pytest.mark.live
def test_airwallex_lists_issuing_cards(crude_config):
    # Issuing cards: snake_case `card_id`, the standard object's `card_number` is
    # masked (crude never calls a full-PAN endpoint). Shape-only; tolerates empty.
    from crude_airwallex.client import AirwallexError

    client = _airwallex_issuing_or_skip(crude_config)
    try:
        items = client.issuing.list_cards(limit=1)
    except AirwallexError as e:
        if e.status in (401, 403, 404):
            pytest.skip("Issuing not enabled on this account")
        raise
    assert isinstance(items, list)
    if items:
        assert items[0].get("card_id")


@pytest.mark.live
def test_airwallex_lists_issuing_transactions(crude_config):
    # Issuing settled card transactions: snake_case `transaction_id`. Shape-only;
    # tolerates empty (the issuing list defaults to a 30-day window).
    from crude_airwallex.client import AirwallexError

    client = _airwallex_issuing_or_skip(crude_config)
    try:
        items = client.issuing.list_transactions(limit=1)
    except AirwallexError as e:
        if e.status in (401, 403, 404):
            pytest.skip("Issuing not enabled on this account")
        raise
    assert isinstance(items, list)
    if items:
        assert items[0].get("transaction_id")
