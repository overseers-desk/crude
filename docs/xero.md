...
# client_secret = "..."  # only for a Web-app registration; selects the confidential flow (see below)
...
Every request carries an `Authorization: Bearer <access_token>` header and, except for `/connections`, an `xero-tenant-id` header naming the resolved organisation. The token endpoint is called one of two ways, selected by config: without a `client_secret`, the app identifies by `client_id` in the request body and proves the exchange with the PKCE code verifier (public client); with one, it authenticates over HTTP Basic with `client_id`/`client_secret` (confidential web app) and sends no PKCE parameters.
