"""Xero Payroll AU API (payroll.xro/2.0) method group over a XeroSession.

One method group for the Australian Payroll product: employees, pay runs, pay
items, timesheets, leave applications, super funds, payroll calendars, and the
read-only payslip and settings. Payroll AU departs from Accounting in two ways
...
* Envelope. A collection comes back under its plural resource key with a
  ``pagination`` sibling (``{"Employees":[...], "pagination":{...}}``). Because
  ``pagination`` is a dict, the single-list-key heuristic in ``_extract_list``
  still resolves to the plural key, so it is reused here for the list/get unwrap
  (verified against the shape) and `paginate` walks the `page` query param.
...
lone non-pagination dict value for those.
"""
...
class PayrollAU:
    ...
    def _one(self, data):
        ...
        fall back to the lone non-`pagination` dict value. Falls back to the whole
        dict for any other shape.
        """
